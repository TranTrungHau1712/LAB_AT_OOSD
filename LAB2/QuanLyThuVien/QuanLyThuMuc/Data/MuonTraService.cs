...
{
    public static class MuonTraService
    {
        public static KetQuaXuLy KiemTraDieuKienMuon(string maDocGia, int soSachMuonMoi)
        {
            ...
            int quaHan = Convert.ToInt32(Db.Scalar(@"
                ...
                WHERE pm.MaDocGia=@Ma AND ct.NgayTraThucTe IS NULL AND pm.NgayHenTra < CAST(GETDATE() AS date)",
        ...
        public static KetQuaXuLy LapPhieuMuon(string maDocGia, string maNhanVien,
            IList<string> maDauSach, DateTime ngayMuon, DateTime ngayHenTra)
        {
            if (maDauSach == null)
        ...
        public static DataTable LaySachDangMuon(string maDocGia)
        {
            return Db.Query(@"
                SELECT ct.MaChiTiet, pm.MaPhieuMuon, pm.MaDocGia, ct.MaDauSach, s.TenSach, pm.NgayMuon, pm.NgayHenTra
                FROM PhieuMuon pm
                JOIN ChiTietPhieuMuon ct ON ct.MaPhieuMuon=pm.MaPhieuMuon
        ...
        public static KetQuaXuLy TraSach(string maChiTiet, string maNhanVien, DateTime ngayTra, string tinhTrang, decimal phiPhat)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    using (SqlCommand up = new SqlCommand(
                        "UPDATE ChiTietPhieuMuon SET NgayTraThucTe=@NgayTra, TinhTrangTra=@TinhTrang " +
                    ...
                    // Sách mất hoặc hư hỏng coi như không còn sẵn sàng cho mượn.
                    if (!mat && !huHong)
                    {
